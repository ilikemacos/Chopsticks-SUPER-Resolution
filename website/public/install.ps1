...
.DESCRIPTION
    This installer downloads the runnable PowerShell/WinForms app from the
    official Universal FrameFX website and installs it per-user. It:
      ...
      5.  Downloads UniversalFrameFX.ps1 and its launcher over HTTPS.
      6.  Verifies the SHA-256 of the download when a checksum is published.
      7.  Installs the files.
     ...
     12.  Optionally launches the app (-Launch, default on).
...
function Get-File([string]$name) {
    ...
    # Optional integrity check: <name>.sha256 next to the file.
    try {
        $sumUrl = "$url.sha256"
        $sumText = (Invoke-WebRequest -Uri $sumUrl -UseBasicParsing -ErrorAction Stop).Content
        ...
        if ($expected) { Write-Ok "SHA-256 verified." }
    } catch {
        Write-Warn2 "No published checksum for $name; skipping integrity check."
    }
    return $dst
}

# --- 5-7. Download & install -------------------------------------------------
$psApp   = Get-File "UniversalFrameFX.ps1"
$launcher = Get-File "UniversalFrameFX.cmd"
